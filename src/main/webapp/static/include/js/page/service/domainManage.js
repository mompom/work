	...
	return {
							...
							if(v.domain_code != 'delete'){
								$("#lst_table_domainInfo tbody").append(
									"<tr class='height_50'>"+
										...
										"<td><button type='button' id='domain_delete_btn' class='btn_st_delete bt02'><span>중지</span></button></td>"+
									"</tr>"
								);
							}else{
								$("#lst_table_domainInfo tbody").append(
				...
				if (response.result) {
					...
					$(response.list).each(function(k,v){
						if(v.use_yn == 'y'){
							...
							"</td>"+
							"<td class='border_right'><input type='text' class='uri_uriInfo width_90p' value='"+v.uri+"' placeholder='등록된 URL이 없습니다.' />"+
							"</td>"+
							"<td class='border_right'><input type='text' class='service_code_uriInfo width_90p' value='"+v.service_code+"' placeholder='ex) ib' /></td>"+
							"<td id='reg_date_uriInfo' class='border_right'>"+v.reg_date+"</td>"+
							"<td>"+
							"<button type='button' id='uri_update_btn' class='btn_st_update bt02'><span>수정</span></button>"+
							"<button type='button' id='uri_delete_btn' class='btn_st_delete bt02'><span>중지</span></button>"+
							"</td>"+
							"</tr>";
							...
							$("#lst_table_urlInfo tbody").append(
								"<tr class='height_50'>"+
									...
									"</td>"+
									"<td class='service_code_uriInfo border_right'>"+v.service_code+"</td>"+
									"<td id='reg_date_uriInfo' class='border_right'>"+v.reg_date+"</td>"+
									"<td>"+
									"<button type='button' id='uri_use_btn' class='btn_st_use bt02'><span>사용</span></button>"+
									"</td>"+
									"</td>"+
								"</tr>"
							);
		...
		uri : {
			...
			use: function(obj){
				...
				ajaxGet("/${map.loanId}/api/domainUriUse", params, root.uri.use_cb, params);
			},
			use_cb : function(response, data){
				if(response.result){
					root.domain.infoGet(data.company_id);
					root.domain.listGet(data.company_id);
					root.domain.selectBox_view(data.company_id);
