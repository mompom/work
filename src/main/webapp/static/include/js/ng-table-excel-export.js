...
var ngTableExcelExport = (function() {
    ...
    var tableToHTML = function(table) {
        ...
        for (i = 0; i < table.rows.length; i=i+1) {
            ...
            for (j = columnStart; j < row.cells.length; j=j+1) {
                ...
                //전체 목록에서는 업체명을 보여주기 위함
                var company_id = $('#company_id').val();
				if(company_id){
					if($(col).attr('class') != 'border_right hide'){
	                	rowData += $(col).context.outerHTML;
	                }
				}else{
					rowData += $(col).context.outerHTML;
				}
                /*if (i === 0) {
                	...
                	//rowData += '<td style="text-align:center; mso-number-format:\'###\';">' + col.textContent.trim() + '</td>';
                	rowData += $(col).context.outerHTML;
                }*/
    ...
    function createDownloadLink(anchor, base64data, exporttype, filename) {
        ...
        if (window.navigator.msSaveBlob) {
            ...
            blob = b64toBlob(base64data, exporttype);
            var uri = window.URL.createObjectURL(blob, exporttype, filename);
            //anchor.href = blobUrl;
        } else {
            var uri = "data:" + exporttype + ";base64," + base64data;
